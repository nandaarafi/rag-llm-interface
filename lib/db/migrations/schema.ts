import { pgTable, foreignKey, uuid, timestamp, text, boolean, varchar, integer, json, unique, primaryKey } from "drizzle-orm/pg-core"
...
export const systemSettings = pgTable("SystemSettings", {
	...
	condition: boolean().default(true).notNull(),
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow(),
	updatedAt: timestamp("updated_at", { mode: 'string' }).defaultNow(),
},
(table) => {
	return {
		systemSettingsDescriptionUnique: unique("SystemSettings_description_unique").on(table.description),
	}
});
